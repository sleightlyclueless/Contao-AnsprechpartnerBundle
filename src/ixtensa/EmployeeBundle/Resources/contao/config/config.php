 ...
 */
 ...
 // BACK END FORM FIELDS, genutzt für eigene Widgets in Inhaltselementen
 // Wir haben in der /EmployeeBundle/Widget/AbtMenu.php, /EmployeeBundle/Widget/EmployeeCheckboxes.php und /EmployeeBundle/Widget/EmployeePicker.php zusätzliche eigene Widgets (InputType Feld für Contao DCA), abgeleitet von der Contao Checkbox und dem Contao Select Menü erstellt. Damit wir diese Felder erstellen und verwenden können müssen wir als aller erstes die neuen Dateien und ihren Typ Contao "zeigen", damit Sie korrekt verwendet und geparst werden. Das machen wir hier über ['BE_FFL']['WidgetName'] = Namespace, wo das Widget konfiguriert wird (Namespace muss dann entsprechend auch in der verwiesenen PHP Datei stehen)
 // Back end form fields kommen aus einem globalen Array $GLOBALS['BE_FFL']['WidgetName']
 $GLOBALS['BE_FFL']['AbtMenu']  = 'ixtensa\\EmployeeBundle\\Widget\\AbtMenu';
 $GLOBALS['BE_FFL']['EmployeeCheckboxes']  = 'ixtensa\\EmployeeBundle\\Widget\\EmployeeCheckboxes';
 $GLOBALS['BE_FFL']['EmployeePicker']  = 'ixtensa\\EmployeeBundle\\Widget\\EmployeePicker';


// BACK END MODULES
// Wir erstellen jetzt neue Backend Module, in denen wir unsere Employee konfigurieren und später übers Frontend ausgeben können.
// Schema: $GLOBALS['Backend Modul']['Überschrift für Modulbereich (wird in languages datei entsprechend übersetzt)']['modulname (wird ebenfalls noch per languages übersetzt)'] = array
// Backend Module sind in der Linken Navigation vom Contao Backend
// Abteilungen – Zentrale Abteilungen für Employee anlegen und dort per Check Widget wählen
 ...
 (
    ...
    'tables'            => array('tl_ixe_departement', 'tl_ixe_departement_lang'),
    // hideInNavigation versteckt das dann im Backend - um Dopplungen im Navigationsmenü zu verhindern , denn wir rufen dieses Backend Modul dann über eine Operation in der Employee DCA auf und wollen nicht für eine Erweiterung zwei Navigationspunkte erstellen - nicht übersichtlich und abhängig
    'hideInNavigation'  => true,

    // 'callback'     => 'ClassName',
    // 'key'          => array('Class', 'method'),
    // 'icon'         => 'path/to/icon.gif',
    // 'stylesheet'   => '/Resources/public/css/zix_employee.css',
    // 'javascript'   => 'path/to/javascript.js'
 );
// Employee – Mitarbeiter als Backend Modul mit den Abteilungen in der Dca als Link integriert und vielen weiteren DCA Feldern konfigurieren und für Frontend Ausgabe vorbereiten
 ...
 // Da wir in dieser Erweiterung auch ein Content Element liefern, mit denen die Mitarbeiter in dem Frontend per Artikel ausgegeben werden können, erstellen wir diese hier nun
 $GLOBALS['TL_CTE']['Ixtensa']['employee'] = 'ixtensa\\EmployeeBundle\\Classes\\Employee';


 // Backend CSS und JS Dateien hinzufügen
 // Zusätzliche Styles (DCA Grids z.B.) werden in zusätzlich definierten Stylesheets für diese Erweiterung in /EmployeeBundle/Resources/public/css/be.css noch einmal ausgeblendet
  if (TL_MODE == 'BE') {
      $GLOBALS['TL_CSS'][] = 'bundles/employee/css/be.css';
  }

 // INFOS zu weiteren möglichen Konfigurationen (nicht relevant für diese Erweiterung konkret)
 // =====================================================================================================================================
